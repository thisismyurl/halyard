 ...
 * copyright paragraph to the `halyard/copyright` source registered here (WP
 ...
 * names derive from HALYARD_SLUG, so a theme installed beside its siblings never
 ...
 */
function halyard_register_bindings(): void {
	if ( ! function_exists( 'register_block_bindings_source' ) ) {
		return;
	}

	register_block_bindings_source(
		HALYARD_SLUG . '/copyright',
		array(
			'label'              => esc_html__( 'Copyright line', 'halyard' ),
			'get_value_callback' => 'halyard_get_copyright_value',
			'uses_context'       => array(),
		)
	);

	register_block_bindings_source(
		HALYARD_SLUG . '/site-strings',
		array(
			'label'              => esc_html__( 'Theme strings', 'halyard' ),
			'get_value_callback' => 'halyard_get_site_string_value',
			'uses_context'       => array(),
		)
	);
...
add_action( 'init', 'halyard_register_bindings' );
 ...
 */
function halyard_get_site_string_value( array $source_args ): string {
	switch ( $source_args['key'] ?? '' ) {
		case 'breaking-label':
			return esc_html__( 'Breaking', 'halyard' );
		case 'home-label':
			return esc_html__( 'Back to the front page', 'halyard' );
		case 'home-url':
			return esc_url( home_url( '/' ) );
		case 'not-found-title':
			return esc_html__( 'Page not found', 'halyard' );
		case 'posts-title':
			$posts_page = (int) get_option( 'page_for_posts' );
			return $posts_page ? esc_html( get_the_title( $posts_page ) ) : esc_html__( 'Latest news', 'halyard' );
	}
	return '';
}
 ...
 */
function halyard_get_copyright_value(): string {
	/**
	 ...
	 */
	$format = (string) apply_filters( HALYARD_SLUG . '/copyright_date_format', 'Y' );
	 ...
	 */
	$copyright = (string) apply_filters( HALYARD_SLUG . '/copyright_text', $copyright );
 ...
 * flow gracefully".
 *
 * A colophon date is not interactive; it is the paper's dateline. Rendering it
 ...
 */
function halyard_get_publication_date_value(): string {
	/**
	 ...
	 */
	$format = (string) apply_filters( HALYARD_SLUG . '/publication_date_format', (string) get_option( 'date_format' ) ); // phpcs:ignore WordPress.NamingConventions.ValidHookName.UseUnderscores
