 ...
 */
function halyard_setup(): void {

	// i18n. The domain is the literal 'halyard' (a constant would break
	// make-pot — see bootstrap.php); the path uses HALYARD_DIR so it travels with a
	// re-skin. The CLI rewrites the literal when it generates a theme.
	load_theme_textdomain( 'halyard', HALYARD_DIR . '/languages' );
	 ...
	 */
	$GLOBALS['content_width'] = (int) apply_filters( HALYARD_SLUG . '/content_width', 720 );
	...
	register_nav_menus(
		(array) apply_filters(
			HALYARD_SLUG . '/register_nav_menus',
			array(
				'primary' => esc_html__( 'Primary Navigation', 'halyard' ),
				'footer'  => esc_html__( 'Footer Navigation', 'halyard' ),
			)
		)
	);
	 ...
	 * @since 1.0.0
	 */
	do_action( HALYARD_SLUG . '/setup' );
}
add_action( 'after_setup_theme', 'halyard_setup' );
 ...
 * plugin is active.
 */
function halyard_woocommerce_support(): void {
...
add_action( 'after_setup_theme', 'halyard_woocommerce_support' );
 ...
 */
function halyard_editor_styles(): void {
	add_editor_style( array( 'assets/css/editor-style.css' ) );
}
add_action( 'after_setup_theme', 'halyard_editor_styles' );
 ...
 */
function halyard_comment_form_field_attributes( array $fields ): array {
	$attributes = array(
...
add_filter( 'comment_form_default_fields', 'halyard_comment_form_field_attributes' );
