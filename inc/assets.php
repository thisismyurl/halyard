 ...
 *
 * Portable: every handle derives from the HALYARD_SLUG constant, so a re-skin needs no
 ...
 *
 * Cache-buster is each file's modification time, not the HALYARD_VERSION constant, so a
 * CSS edit busts caches immediately during iteration without a version bump.
 */
function halyard_enqueue_assets(): void {
...
add_action( 'wp_enqueue_scripts', 'halyard_enqueue_assets' );
 ...
 * an off-origin fetch.
 */
function halyard_preload_fonts(): void {
	/**
	 ...
	 * @since 1.0.0
	 */
	$fonts = (array) apply_filters( HALYARD_SLUG . '/preload_fonts', array() );
	...
	foreach ( $fonts as $font ) {
		...
		printf(
			...
			esc_url( HALYARD_URI . '/' . ltrim( $font, '/' ) )
		);
	}
}
add_action( 'wp_head', 'halyard_preload_fonts', 1 );
