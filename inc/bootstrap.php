 ...
 *
 * This is the ONE place a theme's identity lives. Change the `halyard_` function
 * prefix and the HALYARD_* constants and the entire theme re-prefixes, because
 ...
 * them honest.
 *
 * Why a `halyard_` function prefix and not a namespace (reversed 2026-07-20):
 * this line originally read `namespace Colophon;`, on the reasoning that callbacks
 ...
 * rewrites `halyard_` → `<slug>_`, `HALYARD_` → `<SLUG>_`, and `Halyard_` →
 ...
 * stays a string LITERAL ('halyard'), never the HALYARD_SLUG constant. `wp i18n
 ...
 */
define( 'HALYARD_VERSION', '1.0.0' );

/**
 * Absolute filesystem path to the theme root (no trailing slash).
 */
define( 'HALYARD_DIR', get_template_directory() );

/**
 * Public URL to the theme root (no trailing slash).
 */
define( 'HALYARD_URI', get_template_directory_uri() );
